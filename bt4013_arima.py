...
def myTradingSystem(DATE, OPEN, HIGH, LOW, CLOSE, VOL, OI, P, R, RINFO, exposure, equity, settings):
    ...
    def get_best_arch_model(arima_model, max_q=6, max_p=6):
        test_results = {}
        for p in range(1,max_p): # must start from 1 else error
            ####  Discuss : add in 'o' : asymmetric innoovation?
            for q in range(1,max_q): # must start from 1 else error
    ...
    def get_one_step_forecast(model, new_data):
        ...
        if str(type(model)) == "<class 'arch.univariate.base.ARCHModelResult'>":  # means garch model
            ...
            new_model = arch_model(mean='Zero', y=new_updated_residual,vol='GARCH').fit(starting_values = old_garch_params,
                                                                           update_freq = 0, 
                                                                           disp = 'off')
            ...
            forecast = forecast[0] # i suppose this is forecasting the mean
        return forecast
