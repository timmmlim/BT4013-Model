...
def myTradingSystem(DATE, OPEN, HIGH, LOW, CLOSE, VOL, OI, P, R, RINFO, exposure, equity, settings):
    ...
    def get_best_arch_model(arima_model, max_q=6, max_p=6):
        ...
        for p in range(max_p):
            for q in range(max_q):
                model = arch_model(mean='Zero', y=arima_model.resid, q=q, p=p, vol='EGARCH').fit()
        ...
        best_model = arch_model(mean='Zero', y=arima_model.resid, p=best_p, q=best_q, vol='EGARCH', rescale=True).fit()
